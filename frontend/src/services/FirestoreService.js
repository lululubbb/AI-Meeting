...
import { collection, addDoc, query, orderBy, onSnapshot,Timestamp, doc, updateDoc, getDoc, getDocs, setDoc } from 'firebase/firestore';
...
class FirestoreService {
   ...
   return onSnapshot(q, (querySnapshot) => {
    ...
    async addToMeetingHistory(userId, sessionName, meetingData) {
      try {
          ...
          showSnackBar('会议已在 Firestore 中创建');
          return meetingRef.id;
      } catch (error) {
          console.error('添加到会议历史记录失败:', error);
           showSnackBar('添加到会议历史记录失败:'+error.message);
          throw error;
      }
  }
  ...
  //       console.log('会议信息已更新:', meetingId);
  //     } else {
  //       await setDoc(meetingRef, {
  //         ...data,
  //         createdAt: Timestamp.now() // 加入创建时间
  //       });

  //       console.log('新的会议记录已创建', meetingId);
  //     }
  //   } catch (error) {
  //     console.error('添加/更新会议历史记录失败:', error);
  //     showSnackBar('更新或创建会议时发生错误:'+error.message)
  //     throw error; // *重要*：抛出错误
  //   }
  // }
}
