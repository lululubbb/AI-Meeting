  ...
  history.forEach(msg => {
    if (!msg.file) {
      chatMessagesList.value.push({
        ...
        message: msg.message,
        avatar: store.state.usersInMeeting.find(u => u.userId === msg.sender.userId)?.avatarUrl || defaultAvatar, // *关键修改*: 移除 ?
        file: null,
      ...
      chatMessagesList.value.push({
        ...
        receiverId: msg.receiver.userId,
        // *关键修改*: 移除 ?
        avatar: store.state.usersInMeeting.find(u => u.userId === msg.sender.userId)?.avatarUrl || defaultAvatar, // 从Vuex store中取
        message: null,
        file: {
          name: msg.file.name,
          size: msg.file.size,
...
@media (max-width: 769px) {
  ...
  #action-flow:not(.auto-join-on) .button-container {
    flex-direction: column;
    align-items: stretch;
    gap: 12px;
  }
  ...
  #action-flow .input-group label {
    font-size: 18px; /* 设置输入框字体大小为 16 像素 */
    padding: 5px; /* 可选：增加内边距以提高可读性 */
  }
  ...
  #action-flow .input-group input {
    font-size: 17px;
    padding: 8px;
    height: 45px;
  }
  #action-flow .input-group select {
    font-size: 17px;
    width: 100%;
    height: 45px;
    min-height: 25px;
  ...
  #action-flow .closeBtn {
    font-size: 18px;
    cursor: pointer;
  }

  #action-flow .button-container {
    flex-direction: column;
    align-items: stretch;
    gap: 12px;
    margin-top: 25px;
